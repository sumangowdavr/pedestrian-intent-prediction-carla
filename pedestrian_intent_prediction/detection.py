...
def enhance_image(img: np.ndarray) -> np.ndarray:
    ...
    return img
# 2) Additional preprocessing: histogram equalization in YUV space
...
# 3) Alternative edge overlay: Sobel instead of Canny
# gray = cv2.cvtColor(enhanced, cv2.COLOR_BGR2GRAY)
# sobelx = cv2.Sobel(gray, cv2.CV_64F, 1, 0, ksize=3)
# sobely = cv2.Sobel(gray, cv2.CV_64F, 0, 1, ksize=3)
# sobel = cv2.magnitude(sobelx, sobely).astype(np.uint8)
# enhanced = cv2.addWeighted(enhanced, 0.8, cv2.cvtColor(sobel, cv2.COLOR_GRAY2BGR), 0.2, 0)
# ------------------------ YOLO INIT ------------------------
...
# 4) Morphological closing to fill small gaps in pedestrians’ silhouettes
# kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (5,5))
# enhanced = cv2.morphologyEx(enhanced, cv2.MORPH_CLOSE, kernel)

# 5) Experimented with lower confidence threshold to catch small pedestrians:
# results = model(frame, conf=0.15, iou=0.40, augment=True)
