# Intent-Prediction Demo (self-contained)

The main pipeline needs CARLA RGB/SS frames and YOLO/faster-RCNN weights.
This directory ships a synthetic demo that runs end-to-end with only
`opencv-python`, `numpy`, and `tqdm` (plus optional PIL for the animated GIF).
...
# 2a) Flow-based intent (Farneback optical flow over each bbox)
python3 intent_prediction.py \
...
so speed = 0). Flow-based is 100% because texture inside each ped
means the Farneback flow reflects the actual motion.
